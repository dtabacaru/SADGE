...
public:
  ...
  std::atomic<int> m_render_scale = 5;
  ...
  LcdController(bool screen_on, InterruptReceiver& interrupt_receiver);
  ~LcdController();

  //std::vector<Color>& GetCurrentFrameBuffer()
  //{
  //  return m_frame_buffer;
  //}

  //inline void DumpFrameBufferBin()
  //{
  //  std::ofstream frame_dump("frame_dump.bin", std::ios_base::binary);
  //  frame_dump.write(reinterpret_cast<const char*>(m_frame_buffer.data()), m_frame_buffer.size() * sizeof(Color));
  //}
  ...
  void DrawDoubleFPS(int pos_x, int pos_y);
...
private:
  ...
  bool m_screen_on;
  std::thread      m_screen_thread;
  std::atomic_flag m_screen_initialized;
  std::atomic_flag m_frame_ready_flag;
  std::atomic_bool m_window_closed = false;
